<aside class="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-navy-500 lg:flex"
       role="navigation" aria-label="Sidebar navigasi utama">
    ...
    <div class="border-t border-white/10 px-3 py-3">
        <button x-data="{ dark: localStorage.getItem('dark') === 'true' }"
                ...
                aria-live="polite">
            <svg x-show="!dark" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/>
            </svg>
            <svg x-show="dark" x-cloak class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
...
<aside x-show="sidebarOpen"
    ...
    <div class="fixed inset-0 bg-navy-500/60 backdrop-blur-sm" @click="sidebarOpen = false" aria-hidden="true"></div>
    <div class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy-500 shadow-xl"
        ...
        <div class="flex h-16 items-center justify-between border-b border-white/10 px-6">
            <x-application-logo />
            <button @click="sidebarOpen = false" class="rounded-lg p-1.5 text-white/60 transition-colors hover:text-white" aria-label="Tutup sidebar">
