...
@if ($hasResult)
    ...
    @if (! empty($decodedScores) && isset($decodedScores['EI']))
        <div class="card">
            <h3 class="card-title">{{ __('Dimensi Kepribadian') }}</h3>
    ...
    @elseif (! empty($decodedScores) && isset($decodedScores['D']))
        <div class="card">
            <h3 class="card-title">{{ __('Skor Dimensi') }}</h3>
    ...
    @elseif (! empty($decodedScores))
        <div class="card">
            <h3 class="card-title">{{ __('Skor') }}</h3>
            <div class="mt-4 space-y-4">
                @foreach ($decodedScores as $key => $value)
                    @if (is_array($value))
                        <div>
                            <h4 class="mb-2 text-sm font-semibold text-ink">{{ $key }}</h4>
                            <div class="space-y-2">
                    ...
                    @else
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="text-gray-500">{{ is_numeric($value) ? $value.'%' : $value }}</span>
                        </div>
                    @endif
                @endforeach
            </div>
    ...
    @if (! empty($decodedResult['type']))
        <div class="card py-8 text-center">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">{{ __('Tipe Hasil') }}</p>
    ...
    @if (! empty($decodedResult['iq']) || ! empty($decodedResult['level_title']))
        <div class="card">
            <h3 class="card-title">{{ __('Ringkasan') }}</h3>
            <dl class="mt-4 grid gap-3 sm:grid-cols-2">
                @if (! empty($decodedResult['iq']))
                    <div class="rounded-lg bg-sky-50 px-4 py-3">
                        <dt class="text-xs font-medium uppercase text-sky-600">{{ __('IQ') }}</dt>
                        <dd class="mt-1 text-2xl font-bold text-ink">{{ $decodedResult['iq'] }}</dd>
                    </div>
                @endif
                @if (! empty($decodedResult['percentage']))
                    <div class="rounded-lg bg-gray-50 px-4 py-3">
                        <dt class="text-xs font-medium uppercase text-gray-500">{{ __('Persentase') }}</dt>
                        <dd class="mt-1 text-2xl font-bold text-ink">{{ $decodedResult['percentage'] }}%</dd>
                    </div>
                @endif
                @if (! empty($decodedResult['level_title']))
                    <div class="rounded-lg bg-indigo-50 px-4 py-3 sm:col-span-2">
                        <dt class="text-xs font-medium uppercase text-indigo-600">{{ __('Level') }}</dt>
                        <dd class="mt-1 text-lg font-semibold text-ink">{{ $decodedResult['level_title'] }}</dd>
                    </div>
                @endif
            </dl>
            @if (! empty($decodedResult['advice']))
                <p class="mt-4 text-sm leading-relaxed text-gray-600">{{ $decodedResult['advice'] }}</p>
            @endif
        </div>
    @endif
    ...
    @if (! empty($decodedResult['strengths']))
        <div class="card">
            <h3 class="card-title">{{ __('Kekuatan') }}</h3>
    ...
    @if (! empty($decodedResult['weaknesses']))
        <div class="card">
            <h3 class="card-title">{{ __('Area Pengembangan') }}</h3>
            <ul class="mt-3 grid gap-2 sm:grid-cols-2">
    ...
    @if (! empty($decodedResult['communication_style']))
        <div class="card">
            <h3 class="card-title">{{ __('Gaya Komunikasi') }}</h3>
    ...
    @if (! empty($decodedResult['motivations']))
        <div class="card">
            <h3 class="card-title">{{ __('Motivasi') }}</h3>
    ...
    @if (! empty($decodedResult['fears']))
        <div class="card">
            <h3 class="card-title">{{ __('Ketakutan / Hambatan') }}</h3>
            <div class="mt-3 flex flex-wrap gap-2">
                @foreach ($decodedResult['fears'] as $fear)
                    <span class="rounded-full bg-red-50 px-3 py-1 text-xs font-medium text-red-600">{{ $fear }}</span>
                @endforeach
            </div>
        </div>
    @endif
    ...
    @if (! empty($decodedResult['career_paths']))
        <div class="card">
            <h3 class="card-title">{{ __('Rekomendasi Karier') }}</h3>
            <div class="mt-3 flex flex-wrap gap-2">
...
@else
    <div class="card py-12 text-center">
        ...
        <h3 class="mt-4 text-lg font-semibold text-ink">{{ __('Hasil belum tersedia') }}</h3>
        <p class="mx-auto mt-2 max-w-sm text-sm text-gray-500">{{ __('Data hasil assessment belum dapat ditampilkan.') }}</p>
    </div>
@endif
